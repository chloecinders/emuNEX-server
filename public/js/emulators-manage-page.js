...
import {
  ...
  formStyles, buttonStyles, tableStyles, statusStyles, uploadZoneStyles, tagStyles, modalStyles
...
class EmunexEmulatorsManagePage extends LitElement {
  ...
  static styles = [
    ...
    buttonStyles, tableStyles, statusStyles, uploadZoneStyles, tagStyles, modalStyles,
    css`
      ...
      .checkbox-row { display: flex; align-items: center; gap: var(--spacing-sm); margin-bottom: var(--spacing-md); }
  ...
  _handleSearch(e) {
    ...
    this._searchTimeout = setTimeout(() => {
      this._filtered = this._emulators.filter(e => 
        e.name.toLowerCase().includes(q) || e.console.toLowerCase().includes(q)
      );
    }, 300);
  }
          ...
          <div class="content">
            <div class="header-actions">
              <a href="/emulators/upload" class="popout-btn btn-fit" style="text-decoration: none">
                ...
                <span class="btn-front" style="padding: 6px 12px; font-size: 0.8rem; min-width: auto">+ Upload Emulator</span>
              </a>
            </div>

            <div class="form-group" style="margin-bottom: var(--spacing-md)">
              <input type="text" placeholder="Search emulators by name..." style="font-weight: 700" @input=${this._handleSearch} />
            </div>
            ...
            <div style="overflow-x: auto">
              <table style="margin-top: 0">
                ...
                <tbody>
                  ...
                  ${!this._loading && !this._error && this._filtered.length === 0 ? html`<tr><td colspan="5" style="text-align: center">No emulators found.</td></tr>` : ""}
                  ${this._filtered.map(emu => html`
                    <tr>
                      ...
                      <td class="action-btns">
                        <button class="popout-btn btn-fit btn-info" @click=${() => this.openEdit(emu)}>
                          <span class="btn-edge"></span><span class="btn-front">Edit</span>
                        </button>
                        <button class="popout-btn btn-fit btn-error" @click=${() => this.deleteEmulator(emu.id)}>
          ...
          <div class="content">
            ...
            <form id="editForm" @submit=${this.submitEdit}>
              ...
              <div class="form-group">
                ...
                <input type="text" id="edit-command" .value=${this._editingEmulator.run_command || ""} placeholder="emu.exe {rom}" />
              </div>

              <div class="form-group">
                <label>Binary Name</label>
                <input type="text" id="edit-binary-name" .value=${this._editingEmulator.binary_name || ""} placeholder="emulator.exe" />
              </div>

              <div class="form-group">
                <label>Save Path (optional)</label>
                <input type="text" id="edit-save-path" .value=${this._editingEmulator.save_path || ""} placeholder="/saves/$rom_name.sav" />
              </div>

              <div class="form-group">
                <label>Configuration Files (press Enter to add)</label>
                <div class="tag-system">
                  ${this._editTags.map(t => html`<div class="tag">${t} <span class="tag-remove" @click=${() => this._removeTag(t)}>×</span></div>`)}
                  <div class="tag-input-wrapper">
                    <input type="text" placeholder="config.ini" @keydown=${this._handleTagKeydown} />
                  </div>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <label class="upload-zone ${this._dragover ? 'dragover' : ''}"
                  @dragenter=${this._dragEnter} @dragover=${this._dragOver}
                  @dragleave=${this._dragLeave} @drop=${this._drop}>
                  <div class="upload-icon">↑</div>
                  <div class="upload-info">
                    <div class="upload-text">Upload emulator bundle</div>
                    <div class="file-name">${this._editFileName}</div>
                  </div>
                  <input type="file" id="edit-binary" style="display: none" @change=${this._fileChange} />
                </label>
              </div>
