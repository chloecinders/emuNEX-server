...
import {
  ...
  formStyles, buttonStyles, tableStyles, statusStyles
...
class EmunexUsersPage extends LitElement {
  ...
  static styles = [
    ...
    buttonStyles, tableStyles, statusStyles,
    css`
      ...
      .content-section { margin-bottom: var(--spacing-xxl); }
      .invite-table td.code { font-family: monospace; font-weight: 700; color: var(--color-primary); }
    `
  ];

  constructor() {
    super();
    this._users = []; this._invites = [];
    this._loading = true; this._error = ""; this._status = ""; this._statusType = "";
    this._editingUserId = null; this._editUsername = ""; this._editRole = "";
  }
      ...
      <div class="auth-container">
        <div class="auth-card">
          <header class="card-header">
            ...
            <h1>emuNEX</h1>
            <p class="tagline">User Administration</p>
          </header>
            ...
            <div class="content-section">
              ...
              <div style="overflow-x: auto">
                <table style="margin-top: 0">
                  ...
                  <tbody>
                    ${this._loading ? html`<tr><td colspan="4" style="text-align: center">Loading...</td></tr>` : ""}
                    ${this._error ? html`<tr><td colspan="4" style="text-align: center; color: var(--color-error)">${this._error}</td></tr>` : ""}
                    ${!this._loading && !this._error && this._users.length === 0 ? html`<tr><td colspan="4" style="text-align: center">No users found.</td></tr>` : ""}
                    ...
                    ${this._users.map(u => {
                      ...
                      return html`
                        <tr>
                          ...
                          <td>
                            ${isEditing 
                              ? html`<input type="text" .value=${this._editUsername} @input=${e => this._editUsername = e.target.value} style="padding: 4px; font-size: 0.8rem;" />`
                              : html`<strong style="font-weight: 700">${u.username}</strong>`}
                          </td>
                          <td>
                            ${isEditing
                              ? html`
                                  <select .value=${this._editRole} @change=${e => this._editRole = e.target.value} style="padding: 4px; font-size: 0.8rem;">
                          ...
                          <td class="action-btns">
                            ${isEditing
                              ? html`
                                  <button class="popout-btn btn-fit btn-success" style="--btn-color-primary: #38a169; --btn-color-dark: #276749" @click=${() => this._saveUser(u.id)}>
                                    <span class="btn-edge"></span><span class="btn-front">Save</span>
                                  </button>
                                  <button class="popout-btn btn-fit btn-cancel" @click=${this._cancelEdit}>
                                    <span class="btn-edge"></span><span class="btn-front">Cancel</span>
                                  </button>
                                `
                              : html`
                                  <button class="popout-btn btn-fit btn-info" @click=${() => this._startEdit(u)}>
            ...
            <div class="content-section" style="margin-bottom: 0">
              <div class="header-actions">
                ...
                <button class="popout-btn btn-fit" @click=${this._generateInvite}>
                  <span class="btn-edge"></span><span class="btn-front" style="padding: 6px 12px; font-size: 0.8rem;">+ Generate Code</span>
                </button>
              </div>

              <div style="overflow-x: auto">
                <table class="invite-table" style="margin-top: 0">
                  <thead><tr><th>Code</th><th>Used By</th><th>Created</th><th>Actions</th></tr></thead>
                  <tbody>
                    ${!this._loading && !this._error && this._invites.length === 0 ? html`<tr><td colspan="4" style="text-align: center">No invite codes generated.</td></tr>` : ""}
                    ${this._invites.map(i => html`
                      <tr>
                        <td class="code">${i.code}</td>
                        <td style="font-size: 0.85rem">${i.used_by_username || html`<span style="color: var(--color-text-muted); font-style: italic">Unused</span>`}</td>
                        <td style="font-size: 0.8rem; color: var(--color-text-muted)">${new Date(i.created_at * 1000).toLocaleString()}</td>
                        <td class="action-btns">
                          <button class="popout-btn btn-fit btn-error" @click=${() => this._deleteInvite(i.code)}>
  ...
  _cancelEdit() { this._editingUserId = null; }
