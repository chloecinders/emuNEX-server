...
class EmunexSearchSectionsPage extends LitElement {
    ...
    async fetchData() {
        this._loading = true;
        try {
            const hdrs = { Authorization: localStorage.getItem("token") };
    ...
    async _saveSection() {
        ...
        const data = {
            ...
            order_index: this._isCreating ? this._sections.length : this._sections.find(s => s.id === this._editingSectionId).order_index,
    ...
    renderEditor() {
        ...
        return html`
            <div class="editor-panel">
                <h2 style="margin-bottom: var(--spacing-md)">${this._isCreating ? "Create Section" : "Edit Section"}</h2>
                
                <div class="form-grid">
                    <div class="form-group">
                        <label>Title</label>
                        <input type="text" .value=${this._editTitle} @input=${e => this._editTitle = e.target.value} />
                    </div>
                    <div class="form-group">
                ...
                ${this._editType === "smart" ? html`
                    <div class="form-group" style="margin-bottom: var(--spacing-md)">
                        <label>Filter Type</label>
                        <select .value=${this._editSmartFilter} @change=${e => this._editSmartFilter = e.target.value}>
                ...
                ${this._editType === "category" ? html`
                    <div class="form-group" style="margin-bottom: var(--spacing-md)">
                        <label>Category</label>
                        <select .value=${this._editFilterValue} @change=${e => this._editFilterValue = e.target.value}>
                            ${this._categories.map(c => html`<option value="${c.name}">${c.name}</option>`)}
                        </select>
                    </div>
                ` : ""}

                ${this._editType === "custom" ? html`
                    <div class="form-group" style="margin-bottom: var(--spacing-md)">
                        ...
                        <div style="position: relative;">
                            ...
                            />
                            ${this._romSearchResults.length > 0 ? html`
                                <div class="search-results">
                                    ${this._romSearchResults.map(r => html`
                                        <div class="search-result-item" @click=${() => this._addRomToCustom(r)}>
                    ...
                    <div class="form-group" style="margin-bottom: var(--spacing-md)">
                        ...
                        <div class="rom-list-container">
                            ${this._editRoms.length === 0 ? html`<div style="padding: var(--spacing-sm); color: var(--color-text-muted)">No games strictly specified.</div>` : ""}
                            ${this._editRoms.map((r, idx) => html`
                                <div class="rom-list-item">
                                    <span><strong>${r.title}</strong> (${r.console})</span>
                                    <div class="order-controls">
                                        <button class="popout-btn btn-fit btn-info" @click=${() => this._moveRom(idx, -1)} ?disabled=${idx === 0}>↑</button>
                                        <button class="popout-btn btn-fit btn-info" @click=${() => this._moveRom(idx, 1)} ?disabled=${idx === this._editRoms.length - 1}>↓</button>
                                        <button class="popout-btn btn-fit btn-error" @click=${() => this._removeRomFromCustom(r.id)}>X</button>
                                    </div>
                                </div>
                            `)}
                        </div>
            ...
            <div class="auth-container">
                <div class="auth-card">
                    <header class="card-header">
                        ...
                        <h1>emuNEX</h1>
                        <p class="tagline">Search Sections Administration</p>
                    </header>
                    ...
                    <div class="content">
                        ...
                        <div class="content-section">
                            <div class="header-actions">
                                ...
                                <button class="popout-btn btn-fit btn-success" @click=${this._startCreate} ?disabled=${this._isCreating || this._editingSectionId}>
                                    <span class="btn-edge"></span><span class="btn-front" style="padding: 6px 12px; font-size: 0.8rem;">+ New Section</span>
                                </button>
                            </div>
                            ...
                            <div style="overflow-x: auto">
                                <table style="margin-top: 0">
                                    ...
                                    <tbody>
                                        ...
                                        ${!this._loading && !this._error && this._sections.length === 0 ? html`
                                            <tr><td colspan="5" style="text-align: center">No sections configured.</td></tr>
                                        ` : ""}
                                        
                                        ${this._sections.map((s, idx) => html`
                                            <tr>
                                                <td>
                                                    <div class="order-controls">
                                                        <button class="popout-btn btn-fit btn-info" @click=${() => this._moveSection(idx, -1)} ?disabled=${idx === 0}>↑</button>
                                                        <button class="popout-btn btn-fit btn-info" @click=${() => this._moveSection(idx, 1)} ?disabled=${idx === this._sections.length - 1}>↓</button>
                                                    </div>
                                                </td>
                                                <td><strong>${s.title}</strong></td>
                                                <td>
                                                    ${s.section_type === 'smart' ? 'Smart Group' : s.section_type === 'category' ? 'Category Filter' : 'Custom List'}
                                                </td>
                                                <td style="color: var(--color-text-muted); font-size: 0.85rem">
                                                    ${s.section_type === 'smart' ? s.smart_filter : s.section_type === 'category' ? s.filter_value : `${s.roms ? s.roms.length : 0} item(s)`}
                                                </td>
                                                <td class="action-btns">
                                                    <button class="popout-btn btn-fit btn-info" @click=${() => this._startEdit(s)} ?disabled=${this._editingSectionId === s.id}>
                                                        <span class="btn-edge"></span><span class="btn-front">Edit</span>
                                                    </button>
                                                    <button class="popout-btn btn-fit btn-error" @click=${() => this._deleteSection(s.id)}>
