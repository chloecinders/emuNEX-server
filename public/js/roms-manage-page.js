...
import {
  ...
  formStyles, buttonStyles, tableStyles, statusStyles, uploadZoneStyles, modalStyles
...
class EmunexRomsManagePage extends LitElement {
  ...
  static styles = [
    ...
    buttonStyles, tableStyles, statusStyles, uploadZoneStyles, modalStyles,
    css`
      .auth-container { max-width: 1000px; }
      .header-actions { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-md); }
      ...
      .action-btns .popout-btn { margin: 0; }
  ...
  constructor() {
    super();
    this._roms = []; this._filtered = []; this._consoles = [];
    this._loading = true; this._error = ""; this._status = ""; this._statusType = "";
    this._editModalOpen = false; this._editingRom = null;
    this._editRomFileName = ""; this._editImageFileName = "";
    this._dragoverRom = false; this._dragoverImage = false;
  ...
  _handleSearch(e) {
    ...
    this._searchTimeout = setTimeout(() => {
      this._filtered = this._roms.filter(r => 
        r.title.toLowerCase().includes(q) || 
        r.console.toLowerCase().includes(q) ||
        (r.title_id && r.title_id.toLowerCase().includes(q))
      );
    }, 300);
  }
          ...
          <div class="content">
            <div class="header-actions">
              <a href="/roms/upload" class="popout-btn btn-fit" style="text-decoration: none">
                ...
                <span class="btn-front" style="padding: 6px 12px; font-size: 0.8rem; min-width: auto">+ Upload ROM</span>
              </a>
            </div>

            <div class="form-group" style="margin-bottom: var(--spacing-md)">
              <input type="text" placeholder="Search ROMs by title, ID, or console..." style="font-weight: 700" @input=${this._handleSearch} />
            </div>

            <div style="overflow-x: auto">
              <table style="margin-top: 0">
                <thead>
                  <tr><th>Title ID</th><th>Title</th><th>Console</th><th>Added</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  ${this._loading ? html`<tr><td colspan="5" style="text-align: center">Loading...</td></tr>` : ""}
                  ${this._error ? html`<tr><td colspan="5" style="text-align: center; color: var(--color-error)">${this._error}</td></tr>` : ""}
                  ${!this._loading && !this._error && this._filtered.length === 0 ? html`<tr><td colspan="5" style="text-align: center">No ROMs found.</td></tr>` : ""}
                  ${this._filtered.map(rom => html`
                    <tr>
                      <td style="font-family: monospace; font-size: 0.8rem">${rom.title_id}</td>
                      <td style="font-weight: 700;">${rom.title}</td>
                      <td style="font-weight: 800;">${rom.console}</td>
                      <td style="font-size: 0.8rem; color: var(--color-text-muted)">${new Date(rom.added_at * 1000).toLocaleDateString()}</td>
                      <td class="action-btns">
                        <button class="popout-btn btn-fit btn-info" @click=${() => this.openEdit(rom)}>
                          <span class="btn-edge"></span><span class="btn-front">Edit</span>
                        </button>
                        <button class="popout-btn btn-fit btn-error" @click=${() => this.deleteRom(rom.title_id)}>
            ...
            <form id="editForm" @submit=${this.submitEdit}>
              <div class="grid-2">
                <div class="form-group">
                  ...
                  <input type="text" .value=${this._editingRom.title_id} readonly style="opacity: 0.7; cursor: not-allowed" />
                </div>
                <div class="form-group">
                  <label>Title</label>
              ...
              <div class="form-group">
                ...
                <input type="text" id="edit-md5" .value=${this._editingRom.md5_hash || ""} placeholder="1a2b3c4d..." />
              </div>
              ...
              <div class="grid-3">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="edit-category" .value=${this._editingRom.category || ""} required />
                </div>
                <div class="form-group">
              ...
              <div class="form-group">
                ...
                <label class="upload-zone ${this._dragoverRom ? 'dragover' : ''}"
                  ...
                  @dragleave=${e=>this._dragLeave(e, 'rom')} @drop=${e=>this._drop(e, 'edit-rom', 'editRomFileName')}>
                  <div class="upload-icon">↑</div>
                  <div class="upload-info">
                    <div class="upload-text">Upload new ROM</div>
                    <div class="file-name">${this._editRomFileName}</div>
                  </div>
                  <input type="file" id="edit-rom" style="display: none" @change=${e=>this._fileChange(e, 'editRomFileName')} />
                </label>
              </div>
              ...
              <div class="form-group">
                ...
                <label class="upload-zone ${this._dragoverImage ? 'dragover' : ''}"
                  ...
                  @dragleave=${e=>this._dragLeave(e, 'image')} @drop=${e=>this._drop(e, 'edit-image', 'editImageFileName')}>
                  <div class="upload-icon">↑</div>
                  <div class="upload-info">
                    <div class="upload-text">Upload new cover</div>
                    <div class="file-name">${this._editImageFileName}</div>
                  </div>
                  <input type="file" id="edit-image" style="display: none" @change=${e=>this._fileChange(e, 'editImageFileName')} />
                </label>
              </div>
  ...
  _dragEnter(e, z) { e.preventDefault(); if(z==='rom') this._dragoverRom=true; else this._dragoverImage=true; }
  _dragLeave(e, z) { e.preventDefault(); if(z==='rom') this._dragoverRom=false; else this._dragoverImage=false; }
  _drop(e, id, prop) {
    e.preventDefault(); this._dragoverRom=false; this._dragoverImage=false;
    const input = this.renderRoot.querySelector("#" + id);
    input.files = e.dataTransfer.files;
    if (input.files.length) this[`_${prop}`] = input.files[0].name;
  }
  _fileChange(e, prop) { if (e.target.files.length) this[`_${prop}`] = e.target.files[0].name; }
  ...
  closeModal() { this._editModalOpen = false; }
  ...
  async submitEdit(e) {
    ...
    try {
      ...
      if (romInput.files.length) {
        const d = new FormData(); d.append("rom", romInput.files[0]);
        const r2 = await fetch(`/api/v1/roms/${encodeURIComponent(id)}/file`, {
          method: "POST", headers: { Authorization: localStorage.getItem("token") }, body: d
        });
      ...
      if (imgInput.files.length) {
        const d = new FormData(); d.append("image", imgInput.files[0]);
