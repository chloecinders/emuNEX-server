...
import {
  ...
  formStyles, buttonStyles, statusStyles, uploadZoneStyles, tagStyles
...
class EmunexEmulatorPage extends LitElement {
  ...
  static styles = [
    ...
    buttonStyles, statusStyles, uploadZoneStyles, tagStyles,
    css`
      .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-md); }
      .checkbox-row { display: flex; align-items: center; gap: var(--spacing-sm); margin: var(--spacing-sm) 0; }
            ...
            <form id="emulatorForm" @submit=${this._handleSubmit}>
              ...
              <div class="form-group">
                ...
                <div class="tag-system">
                  ${this._tags.map(t => html`<div class="tag">${t} <span class="tag-remove" @click=${() => this._removeTag(t)}>×</span></div>`)}
                  <div class="tag-input-wrapper">
                    <input type="text" placeholder="Type filename and press Enter" @keydown=${this._handleTagKeydown} />
                  </div>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <label class="upload-zone ${this._dragover ? 'dragover' : ''}"
                  ...
                  <input type="file" id="binary_file" name="binary_file" required style="display: none" @change=${this._fileChange} />
                </label>
              </div>
